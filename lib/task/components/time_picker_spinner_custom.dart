...
DateTime? selectedDateForNotification;
...
class TimePickerSpinnerCustom extends StatefulWidget {
  final DateTime? time;
  ...
  _TimePickerSpinnerCustomState createState() => _TimePickerSpinnerCustomState();
}

class _TimePickerSpinnerCustomState extends State<TimePickerSpinnerCustom> {
  ...
  DateTime getDateTime() {
    ...
    DateTime b = DateTime(currentTime!.year, currentTime!.month, currentTime!.day, hour, minute, second);
    selectedDateForNotification = b;
    //formattedTime = StudyUILogic().getTime(b);
    //formattedDateTime = StudyUILogic().getTime(b);
      ...
      child: Column(
        children: [
          Padding(
            ...
            child: Center(child: Text("Set time", style: theme.bodyMedium,)),
          ),
          Row(
          ...
          widget.isReminder == true?Container():Container(
            margin: EdgeInsets.symmetric(horizontal: kMargin,vertical: kMargin),
            child: SizedBox(
                width: double.infinity,
                child: CustomButton(onPressed: (){ Navigator.pop(context);}, title: "Done")),
          ),
        ],
      ),
    );
      ...
      onNotification: (scrollNotification) {
        if (scrollNotification is UserScrollNotification) {
          ...
          setState(() {
            ...
            //currentSelectedAPIndex = (apController.offset / 20).round() + 1;
            isAPScrolling = true;
          });
        }
        return true;
